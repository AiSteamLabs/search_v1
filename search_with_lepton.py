...
import redis
from search_engine import ModuleNamesMap
...
class myKV:
    ...
    def connect_to_redis_server(self):
        ...
        print(self.redis_server_host, self.redis_server_port)
        self.redis = redis.Redis(host=self.redis_server_host, port=self.redis_server_port)
        print(self.redis)
...
class RAG(Photon):
    ...
    def init(self):
        ...
        if self.backend in ModuleNamesMap: 
            searchEngine = ModuleNamesMap[self.backend]
            self.search_function = searchEngine().search       
        else:
            raise RuntimeError("Backend must be LEPTON, BING, GOOGLE, EXAI.")
        
        self.model = os.environ["LLM_MODEL"] or "llama3-70b"
        ...
        useMyKV = to_bool(os.getenv("useMyKV"))
        if useMyKV:
            self.kv = myKV(
